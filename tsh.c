...
int parseline(const char *cmdline, char **argv) 
{
    ...
    while (delim) {
	argv[argc++] = buf;
	*delim = '\0';//set the the end of the argument to EOF
	buf = delim + 1;
...
{
	int stat;
	pid_t pidS;//the target process id
	int jidS = 0;//the target job id
	struct job_t* jobS;//the targert job
	char * id;//the pointer to the start of the number
	int jorp ;//check if the number input is a jid or pid
	...
	if((int)*id>57||(int)*id<48){
		printf("argument must be a PID or %%jobid\n");
		return;
	}

	//get the process id and job id
	if(jorp){
		jidS = atoi(id);
		if((jobS=getjobjid(jobs,jidS))==NULL){
		...
		pidS = atoi(id);
		if((jobS = getjobpid(jobs,jidS))==NULL){
